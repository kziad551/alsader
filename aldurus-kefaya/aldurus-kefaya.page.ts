...
@Component({
  selector: 'app-aldurus-kefaya',
  templateUrl: './aldurus-kefaya.page.html',
  styleUrls: ['./aldurus-kefaya.page.scss'],
})
export class AldurusKefayaPage implements OnInit {
...
constructor(private nativePageTransitions: NativePageTransitions,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'aldurus-kefaya');
        this.storage.get('aldurus-kefaya-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('aldurus-kefaya-mp3', link);
    }
    forwardLink(){
        this.storage.get('aldurus-kefaya-mp3').then((link) => {
            var options = {
...
function getXMLDataVoice( itemname:string ) {
  ...
  try {
    request.open('GET', 'assets/aldurus-mp3/aldurus-kefaya-mp3.xml', false);
    request.send(null); 
  } catch (err) {  
    return '';
  }
