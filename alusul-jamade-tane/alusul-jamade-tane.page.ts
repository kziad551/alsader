...
@Component({
  selector: 'app-alusul-jamade-tane',
  templateUrl: './alusul-jamade-tane.page.html',
  styleUrls: ['./alusul-jamade-tane.page.scss'],
})
export class AlusulJamadeTanePage implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-jamade-tane-1416&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-jamade-tane');
        this.storage.get('alusul-jamade-tane-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-jamade-tane-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-jamade-tane-mp3').then((link) => {
            var options = {
