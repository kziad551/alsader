...
@Component({
  ...
  styleUrls: ['./takrerat-baad.page.scss'],
})
export class TakreratBaadPage implements OnInit {
  ...
  ionViewWillEnter(){
    ...
    this.storage.set('page', 'takrerat-baad');
    this.storage.get('pdfLink14').then(link => {
      if(link){
  ...
  getlink(link:string){
    this.storage.set('pdfLink14', link);
  }
  forwardLink(){
    this.storage.get('pdfLink14').then((link) => {
