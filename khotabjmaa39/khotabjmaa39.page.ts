...
@Component({
  selector: 'app-khotabjmaa39',
  templateUrl: './khotabjmaa39.page.html',
  styleUrls: ['./khotabjmaa39.page.scss'],
})
export class Khotabjmaa39Page implements OnInit {
  private data:any = [];  
  public content:any[] = [];
  ...
  ngOnInit() { 
    this.storage.get('language-usingg').then(linkd => {
     if( linkd == 'ar'){
        this.data = [];
        this.content = [];
        const url= 'https://strapi.alsader.net/api/khotab-al-jomaa-docs?filters[id][$eq]=39?filters[khotab-al-jomaa-doc-cats][title][$eq]=khotab-docs&locale=ar-IQ'
        ...
        this.data = [];
        this.content = [];
        const url= 'https://strapi.alsader.net/api/khotab-al-jomaa-docs?filters[id][$eq]=39?filters[khotab-al-jomaa-doc-cats][title][$eq]=khotab-docs&locale=fa-IR'
        this.http.get(url).subscribe((res)=>{
          this.data = res
          var i =0;
          for ( i=0; i< this.data.data.length; i++ ) {
