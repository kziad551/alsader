...
@Component({
  selector: 'app-khotabjmaa20',
  templateUrl: './khotabjmaa20.page.html',
  styleUrls: ['./khotabjmaa20.page.scss'],
})
export class Khotabjmaa20Page implements OnInit {
  private data:any = [];  
  public content:any[] = [];
  ...
  ngOnInit() { 
       this.storage.get('language-usingg').then(linkd => {
        if( linkd == 'ar'){
           this.data = [];
           this.content = [];
           const url= 'https://strapi.alsader.net/api/khotab-al-jomaa-docs?filters[id][$eq]=20?filters[khotab-al-jomaa-doc-cats][title][$eq]=khotab-docs&locale=ar-IQ'
           ...
           this.data = [];
           this.content = [];
           const url= 'https://strapi.alsader.net/api/khotab-al-jomaa-docs/159'
           this.http.get(url).subscribe((res)=>{
             this.data = res
             var i =0;
           for ( i=0; i< this.data.data.length; i++ ) {
           ...
           this.data = [];
           this.content = [];
           const url= 'https://strapi.alsader.net/api/khotab-al-jomaa-docs/65'
           this.http.get(url).subscribe((res)=>{
             this.data = res
             var i =0;
             for ( i=0; i< this.data.data.length; i++ ) {
