...
@Component({
  selector: 'app-alusul-zel7eja1417',
  templateUrl: './alusul-zel7eja1417.page.html',
  styleUrls: ['./alusul-zel7eja1417.page.scss'],
})
export class AlusulZel7eja1417Page implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-zulheja-1417&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-zel7eja1417');
        this.storage.get('alusul-zel7eja1417-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-zel7eja1417-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-zel7eja1417-mp3').then((link) => {
            var options = {
