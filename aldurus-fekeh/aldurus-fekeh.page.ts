...
@Component({
  selector: 'app-aldurus-fekeh',
  templateUrl: './aldurus-fekeh.page.html',
  styleUrls: ['./aldurus-fekeh.page.scss'],
})
export class AldurusFekehPage implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bhaes-kharej-fekeh&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'aldurus-fekeh');
        this.storage.get('aldurus-fekeh-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('aldurus-fekeh-mp3', link);
    }
    forwardLink(){
        this.storage.get('aldurus-fekeh-mp3').then((link) => {
            var options = {
