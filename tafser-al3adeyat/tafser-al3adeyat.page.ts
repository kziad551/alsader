...
@Component({
  selector: 'app-tafser-al3adeyat',
  templateUrl: './tafser-al3adeyat.page.html',
  styleUrls: ['./tafser-al3adeyat.page.scss'],
})
export class TafserAl3adeyatPage implements OnInit {
  ngOnInit() { 
         const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=mohadarat-tafsir-tafsir-al3adeyat&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'tafser-al3adeyat');
        this.storage.get('tafser-al3adeyat-mp3').then(link => {
            if(link){
    ...
    streamaudio(url: string){
        this.storage.set('tafser-al3adeyat-mp3', url);
        var options = {
            bgColor: "#000000",
            bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
            bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//     try {
//         request.open('GET', 'assets/aldurus-mp3/tafser/tafser-al3adeyat.xml', false);
//         request.send(null);
//     } catch (err) {
//         return '';
//     }
