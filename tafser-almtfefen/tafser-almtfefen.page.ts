...
@Component({
  selector: 'app-tafser-almtfefen',
  templateUrl: './tafser-almtfefen.page.html',
  styleUrls: ['./tafser-almtfefen.page.scss'],
})
export class TafserAlmtfefenPage implements OnInit {
  ...
  public content:any[] = [];
  ngOnInit() { 
        const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=mohadarat-tafsir-tafsir-almutafefen&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'tafser-almtfefen');
        this.storage.get('tafser-almtfefen-mp3').then(link => {
            if(link){
    ...
    streamaudio(url: string){
        this.storage.set('tafser-almtfefen-mp3', url);
        var options = {
            bgColor: "#000000",
            bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
            bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//     try {
//         request.open('GET', 'assets/aldurus-mp3/tafser/tafser-almutafefen.xml', false);
//         request.send(null);
//     } catch (err) {
//         return '';
//     }
