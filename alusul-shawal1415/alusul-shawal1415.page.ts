...
@Component({
  selector: 'app-alusul-shawal1415',
  templateUrl: './alusul-shawal1415.page.html',
  styleUrls: ['./alusul-shawal1415.page.scss'],
})
export class AlusulShawal1415Page implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-shawal-1415&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-shawal1415');
        this.storage.get('alusul-shawal1415').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-shawal1415', link);
    }
    forwardLink(){
        this.storage.get('alusul-shawal1415').then((link) => {
            var options = {
