...
@Component({
  selector: 'app-alusul-sefer1417',
  templateUrl: './alusul-sefer1417.page.html',
  styleUrls: ['./alusul-sefer1417.page.scss'],
})
export class AlusulSefer1417Page implements OnInit {
    forwardshow: boolean = true;
    private data:any = [];
    public title:any[] = [];
    public content:any[] = [];
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-sefer-1417&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-sefer1417');
        this.storage.get('alusul-sefer1419-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-sefer1417-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-sefer1417-mp3').then((link) => {
            var options = {
                bgColor: "#000000",
