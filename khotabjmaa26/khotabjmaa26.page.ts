...
@Component({
  selector: 'app-khotabjmaa26',
  templateUrl: './khotabjmaa26.page.html',
  styleUrls: ['./khotabjmaa26.page.scss'],
})
export class Khotabjmaa26Page implements OnInit {
  private data:any = [];  
  public content:any[] = [];
  ...
  ngOnInit() { 
    this.storage.get('language-usingg').then(linkd => {
     if( linkd == 'ar'){
        this.data = [];
        this.content = [];
        const url= 'https://strapi.alsader.net/api/khotab-al-jomaa-docs?filters[id][$eq]=26?filters[khotab-al-jomaa-doc-cats][title][$eq]=khotab-docs&locale=ar-IQ'
        ...
        this.data = [];
        this.content = [];
        const url= 'https://strapi.alsader.net/api/khotab-al-jomaa-docs?filters[id][$eq]=71?filters[khotab-al-jomaa-doc-cats][title][$eq]=khotab-docs&locale=fa-IR'
        this.http.get(url).subscribe((res)=>{
          this.data = res
          var i =0;
          for ( i=0; i< this.data.data.length; i++ ) {
