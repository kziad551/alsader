...
@Component({
  selector: 'app-alusul-jamade-awal1417',
  templateUrl: './alusul-jamade-awal1417.page.html',
  styleUrls: ['./alusul-jamade-awal1417.page.scss'],
})
export class AlusulJamadeAwal1417Page implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-jamade-awal-1417&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-jamade-awal1417');
        this.storage.get('alusul-jamade-awal1417-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-jamade-awal1417-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-jamade-awal1417-mp3').then((link) => {
            var options = {
