...
@Component({
  selector: 'app-tafser-naba2',
  templateUrl: './tafser-naba2.page.html',
  styleUrls: ['./tafser-naba2.page.scss'],
})
export class TafserNaba2Page implements OnInit {
    forwardshow: boolean = true;
  private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
  ngOnInit() { 
        const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=mohadarat-tafsir-tafsir-alnaba2&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'tafser-naba2');
        this.storage.get('tafser-naba2-mp3').then(link => {
            if(link){
    ...
    streamaudio(url: string){
        this.storage.set('tafser-naba2-mp3', url);
        var options = {
            bgColor: "#000000",
            bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
            bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//     try {
//         request.open('GET', 'assets/aldurus-mp3/tafser/tafser-alnaba2.xml', false);
//         request.send(null);
//     } catch (err) {
//         return '';
//     }
