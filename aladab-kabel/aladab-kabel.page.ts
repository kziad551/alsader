...
@Component({
  selector: 'app-aladab-kabel',
  templateUrl: './aladab-kabel.page.html',
  styleUrls: ['./aladab-kabel.page.scss'],
})
export class AladabKabelPage implements OnInit {
  ...
  ionViewWillEnter(){
  this.storage.clear();
  this.storage.set('page', 'aladab-kabel');
}
...
function getXMLDataPDF( itemname:string ) {
  ...
  try {
    request.open('GET', 'assets/mawsuaat-xml/الادب-قبل-الاستشهاد.xml', false);
    request.send(null);
  } catch (err) {
    return '';
  }
