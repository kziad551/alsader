...
@Component({
  selector: 'app-alusul-sefer1418',
  templateUrl: './alusul-sefer1418.page.html',
  styleUrls: ['./alusul-sefer1418.page.scss'],
})
export class AlusulSefer1418Page implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-sefer-1418&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-sefer1418');
        this.storage.get('alusul-sefer1418-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-sefer1418-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-sefer1418-mp3').then((link) => {
            var options = {
