...
@Component({
  selector: 'app-tafser-alhamza',
  templateUrl: './tafser-alhamza.page.html',
  styleUrls: ['./tafser-alhamza.page.scss'],
})
export class TafserAlhamzaPage implements OnInit {
      private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
    forwardshow: boolean = true;
  ngOnInit() { 
        const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=mohadarat-tafsir-tafsir-alhamza&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'tafser-alhamza');
        this.storage.get('tafser-alhamza-mp3').then(link => {
            if(link){
    ...
    streamaudio(url: string){
        this.storage.set('tafser-alhamza-mp3', url);
        var options = {
            bgColor: "#000000",
            bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
            bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//     try {
//         request.open('GET', 'assets/aldurus-mp3/tafser/tafser-alhamza.xml', false);
//         request.send(null);
//     } catch (err) {
//         return '';
//     }
