...
@Component({
  selector: 'app-alusul-sefer1419',
  templateUrl: './alusul-sefer1419.page.html',
  styleUrls: ['./alusul-sefer1419.page.scss'],
})
export class AlusulSefer1419Page implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-sefer-1419&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-sefer1419');
    ...
    getlink(link:string){
        this.storage.set('alusul-sefer1419-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-sefer1419-mp3').then((link) => {
            var options = {
