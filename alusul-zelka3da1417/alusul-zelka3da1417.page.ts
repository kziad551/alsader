...
@Component({
  selector: 'app-alusul-zelka3da1417',
  templateUrl: './alusul-zelka3da1417.page.html',
  styleUrls: ['./alusul-zelka3da1417.page.scss'],
})
export class AlusulZelka3da1417Page implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-zulke3da-1417&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-zelka3da1417');
        this.storage.get('alusul-zelka3da1417-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-zelka3da1417-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-zelka3da1417-mp3').then((link) => {
            var options = {
