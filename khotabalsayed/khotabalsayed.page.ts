...
@Component({
  selector: 'app-khotabalsayed',
  templateUrl: './khotabalsayed.page.html',
  styleUrls: ['./khotabalsayed.page.scss'],
})
export class KhotabalsayedPage implements OnInit {
...
getTopics(ev:any){
 this.searchClicked= true;
  ...
  if(serVal && serVal.trim() !=''){
    ...
    forward(){
        this.storage.get('page-khotab-alsayed').then((val) => {
...
function getXMLDataVoice( itemname:string ) {
  ...
  try {
    request.open('GET', 'assets/search-xml/search-khotab-jmaa.xml', false);
    request.send(null);
  } catch (err) {
    return '';
  }
