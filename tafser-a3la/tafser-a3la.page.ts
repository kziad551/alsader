...
@Component({
  selector: 'app-tafser-a3la',
  templateUrl: './tafser-a3la.page.html',
  styleUrls: ['./tafser-a3la.page.scss'],
})
export class TafserA3laPage implements OnInit {
 private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
  ngOnInit() { 
        const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bmohadarat-tafsir-tafsir-a3la&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'tafser-a3la');
        this.storage.get('tafser-a3la-mp3').then(link => {
            if(link){
    ...
    streamaudio(url: string){
        this.storage.set('tafser-a3la-mp3', url);
        var options = {
            bgColor: "#000000",
            bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
            bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//     try {
//         request.open('GET', 'assets/aldurus-mp3/tafser/tafser-a3la.xml', false);
//         request.send(null);
//     } catch (err) {
//         return '';
//     }
