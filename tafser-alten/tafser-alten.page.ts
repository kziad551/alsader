...
@Component({
  selector: 'app-tafser-alten',
  templateUrl: './tafser-alten.page.html',
  styleUrls: ['./tafser-alten.page.scss'],
})
export class TafserAltenPage implements OnInit {
 ...
 private data:any = [];
  ...
  ngOnInit() {
     const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=mohadarat-tafsir-tafsir-alten&populate=*'
  ...
  ionViewWillEnter(){
    ...
    this.storage.set('page-aldurus', 'tafser-alten');
    this.storage.get('tafser-alten-mp3').then(link => {
      if(link){
  ...
  streamaudio(url: string){
    this.storage.set('tafser-alten-mp3', url);
    var options = {
      bgColor: "#000000",
      bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
      bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//   try {
//     request.open('GET', 'assets/aldurus-mp3/tafser/tafser-alten.xml', false);
//     request.send(null);
//   } catch (err) {
//     return '';
//   }
