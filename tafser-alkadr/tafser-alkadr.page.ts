...
@Component({
  selector: 'app-tafser-alkadr',
  templateUrl: './tafser-alkadr.page.html',
  styleUrls: ['./tafser-alkadr.page.scss'],
})
export class TafserAlkadrPage implements OnInit {
     private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
    forwardshow: boolean = true;
  ngOnInit() { 
              const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=mohadarat-tafsir-tafsir-alkader&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'tafser-alkadr');
        this.storage.get('tafser-alkadr-mp3').then(link => {
            if(link){
    ...
    streamaudio(url: string){
        this.storage.set('tafser-alkadr-mp3', url);
        var options = {
            bgColor: "#000000",
            bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
            bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//     try {
//         request.open('GET', 'assets/aldurus-mp3/tafser/tafser-alkadar.xml', false);
//         request.send(null);
//     } catch (err) {
//         return '';
//     }
