...
@Component({
  selector: 'app-alusul-zel7eja1415',
  templateUrl: './alusul-zel7eja1415.page.html',
  styleUrls: ['./alusul-zel7eja1415.page.scss'],
})
export class AlusulZel7eja1415Page implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-zulheja-1415&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-zel7eja1415');
        this.storage.get('alusul-zel7eja1415-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-zel7eja1415-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-zel7eja1415-mp3').then((link) => {
            var options = {
