...
@Component({
  selector: 'app-kefaya1418',
  templateUrl: './kefaya1418.page.html',
  styleUrls: ['./kefaya1418.page.scss'],
})
export class Kefaya1418Page implements OnInit {
  ...
  ionViewWillEnter(){
    ...
    this.storage.set('aldurus-kefaya', 'kefaya1418');
    this.storage.get('kefaya1418').then(link => {
      if(link){
  ...
  getlink(link:string){
    this.storage.set('kefaya1418', link);
  }
  moveToPage(pageName) {
...
function getXMLDataVoice( itemname:string ) {
  ...
  try {
    request.open('GET', 'assets/aldurus-mp3/aldurus-kefaya1418/titles.xml', false);
    request.send(null);
  } catch (err) {
    return '';
  }
