...
@Component({
  selector: 'app-alusul-rajab1419',
  templateUrl: './alusul-rajab1419.page.html',
  styleUrls: ['./alusul-rajab1419.page.scss'],
})
export class AlusulRajab1419Page implements OnInit {
  private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-rajab-1419&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-rajab1419');
        this.storage.get('alusul-rajab1419-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-rajab1419-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-rajab1419-mp3').then((link) => {
            var options = {
