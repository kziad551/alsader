...
@Component({
  selector: 'app-alusul-akaed-shajzrat',
  templateUrl: './alusul-akaed-shajzrat.page.html',
  styleUrls: ['./alusul-akaed-shajzrat.page.scss'],
})
export class AlusulAkaedShajzratPage implements OnInit {
  ...
  constructor(private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-akaed-shajzrat');
        this.storage.get('alusul-akaed-shajzrat-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-akaed-shajzrat-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-akaed-shajzrat-mp3').then((link) => {
            var options = {
  ...
  function getXMLDataVoice( itemname:string ) {
    ...
    try {
      request.open('GET', 'assets/alusul-akaed-shazrat-mp3/shazrat-mp3.xml', false);
      request.send(null);
    } catch (err) {
      return '';
    }
