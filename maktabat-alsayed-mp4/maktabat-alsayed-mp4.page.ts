  ...
  ngOnInit() {
    ...
    this.storage.get('language-usingg').then(linkd => {
      ...
      if( linkd == 'ar'){
         this.data = [];
         this.content = [];
         const url= 'https://strapi.alsader.net/api/almaktabas?filters[almaktaba_cat][title][$eq]=maktaba-khasa-mp4&locale=ar-IQ'
         ...
         this.data = [];
         this.content = [];
         const url= 'https://strapi.alsader.net/api/almaktabas?filters[almaktaba_cat][title][$eq]=maktaba-khasa-mp4&locale=en'
         this.http.get(url).subscribe((res)=>{
           this.data = res
           var i =0;
           for ( i=0; i< this.data.data.length; i++ ) {
         ...
         this.data = [];
         this.content = [];
         const url= 'https://strapi.alsader.net/api/almaktabas?filters[almaktaba_cat][title][$eq]=maktaba-khasa-mp4&locale=fa-IR'
         this.http.get(url).subscribe((res)=>{
           this.data = res
           var i =0;
           for ( i=0; i< this.data.data.length; i++ ) {
