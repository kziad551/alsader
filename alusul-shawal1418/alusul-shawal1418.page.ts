...
@Component({
  selector: 'app-alusul-shawal1418',
  templateUrl: './alusul-shawal1418.page.html',
  styleUrls: ['./alusul-shawal1418.page.scss'],
})
export class AlusulShawal1418Page implements OnInit {
  private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
    forwardshow: boolean = true;
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-shawal-1418&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-shawal1418');
        this.storage.get('alusul-shawal1418-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-shawal1418-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-shawal1418-mp3').then((link) => {
            var options = {
