...
@Component({
  selector: 'app-tafser-alnaze3at',
  templateUrl: './tafser-alnaze3at.page.html',
  styleUrls: ['./tafser-alnaze3at.page.scss'],
})
export class TafserAlnaze3atPage implements OnInit {
    forwardshow: boolean = true;
   private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
  ngOnInit() { 
     const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=mohadarat-tafsir-tafsir-alnaze3at&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'tafser-alnaze3at');
        this.storage.get('tafser-alnaze3at-mp3').then(link => {
            if(link){
    ...
    streamaudio(url: string){
        this.storage.set('tafser-alnaze3at-mp3', url);
        var options = {
            bgColor: "#000000",
            bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
            bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//     try {
//         request.open('GET', 'assets/aldurus-mp3/tafser/tafser-alnaze3at.xml', false);
//         request.send(null);
//     } catch (err) {
//         return '';
//     }
