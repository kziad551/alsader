...
@Component({
  selector: 'app-alusul-shaaban1418',
  templateUrl: './alusul-shaaban1418.page.html',
  styleUrls: ['./alusul-shaaban1418.page.scss'],
})
export class AlusulShaaban1418Page implements OnInit {
 ...
 ngOnInit() { 
  ...
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-shaaban-1418&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-shaaban1418');
        this.storage.get('alusul-shaaban1418-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-shaaban1418-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-shaaban1418-mp3').then((link) => {
            var options = {
