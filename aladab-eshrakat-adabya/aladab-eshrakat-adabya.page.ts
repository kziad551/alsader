...
@Component({
  selector: 'app-aladab-eshrakat-adabya',
  templateUrl: './aladab-eshrakat-adabya.page.html',
  styleUrls: ['./aladab-eshrakat-adabya.page.scss'],
})
export class AladabEshrakatAdabyaPage implements OnInit {
  ...
  ionViewWillEnter(){
    ...
    this.storage.set('page', 'adab-eshrakat-adabya');
    this.storage.get('pdfLink99').then(link => {
      if(link){
  ...
  getlink(link:string){
    this.storage.get('pdfLink99').then(value => {
      this.storage.remove(value);
    });
    this.storage.set('pdfLink99', link);
  }
  forwardLink(){
    this.storage.get('pdfLink99').then((link) => {
  ...
  ngOnInit() {
    const url= 'https://strapi.alsader.net/api/pdfs?filters[pdf_category][title][$eq]=ishrakat-adabya&populate=*'
