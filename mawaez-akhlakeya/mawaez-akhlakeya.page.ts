...
@Component({
  selector: 'app-mawaez-akhlakeya',
  templateUrl: './mawaez-akhlakeya.page.html',
  styleUrls: ['./mawaez-akhlakeya.page.scss'],
})

export class MawaezAkhlakeyaPage implements OnInit {
    ...
    forward(){
        this.storage.get('page-mawaez-akhlakeya').then((val) => {
  ...
  function getXMLDataVoice( itemname:string ) {
    ...
    try {
      request.open('GET', 'assets/search-xml/search-mawaez.xml', false);
      request.send(null);
    } catch (err) {
      return '';
    }
