...
@Component({
  selector: 'app-alusul-zel7eja',
  templateUrl: './alusul-zel7eja.page.html',
  styleUrls: ['./alusul-zel7eja.page.scss'],
})
export class AlusulZel7ejaPage implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-zulheja-1416&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-zel7eja');
        this.storage.get('alusul-zel7eja-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-zel7eja-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-zel7eja-mp3').then((link) => {
            var options = {
