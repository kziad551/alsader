...
@Component({
  selector: 'app-alusul-shaher-muharam1418',
  templateUrl: './alusul-shaher-muharam1418.page.html',
  styleUrls: ['./alusul-shaher-muharam1418.page.scss'],
})
export class AlusulShaherMuharam1418Page implements OnInit {
  public title;
  public link;
  public voice = [];
...
constructor(private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-shaher-muharam1418');
        this.storage.get('alusul-shaher-muharam1418-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-shaher-muharam1418-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-shaher-muharam1418-mp3').then((link) => {
            var options = {
...
function getXMLDataVoice( itemname:string ) {
  ...
  try {
    request.open('GET', 'assets/alusul-1418-mp3/muharam-mp3.xml', false);
    request.send(null); 
  } catch (err) {  
    return '';
  }
