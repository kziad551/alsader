...
@Component({
  selector: 'app-alusul-shaher-muharam1419',
  templateUrl: './alusul-shaher-muharam1419.page.html',
  styleUrls: ['./alusul-shaher-muharam1419.page.scss'],
})
export class AlusulShaherMuharam1419Page implements OnInit {
  private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
 ...
 ngOnInit() { 
  ...
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-muharam-1419&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-shaher-muharam1419');
        this.storage.get('alusul-shaher-muharam1419-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-shaher-muharam1419-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-shaher-muharam1419-mp3').then((link) => {
            var options = {
