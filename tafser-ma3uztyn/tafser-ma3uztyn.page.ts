...
@Component({
  selector: 'app-tafser-ma3uztyn',
  templateUrl: './tafser-ma3uztyn.page.html',
  styleUrls: ['./tafser-ma3uztyn.page.scss'],
})
export class TafserMa3uztynPage implements OnInit {
    forwardshow: boolean = true;
 private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
  ngOnInit() { 
        const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=mohadarat-tafsir-tafsir-alma3uzatayn&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'tafser-ma3uztyn');
        this.storage.get('tafser-ma3uztyn-mp3').then(link => {
            if(link){
    ...
    streamaudio(url: string){
        this.storage.set('tafser-ma3uztyn-mp3', url);
        var options = {
            bgColor: "#000000",
            bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
            bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//     try {
//         request.open('GET', 'assets/aldurus-mp3/tafser/tafser-alm3uzatyn.xml', false);
//         request.send(null);
//     } catch (err) {
//         return '';
//     }
