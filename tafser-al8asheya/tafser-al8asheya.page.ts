...
@Component({
  selector: 'app-tafser-al8asheya',
  templateUrl: './tafser-al8asheya.page.html',
  styleUrls: ['./tafser-al8asheya.page.scss'],
})
export class TafserAl8asheyaPage implements OnInit {
    private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
  ngOnInit() { 
          const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=mohadarat-tafsir-tafsir-al8asheya&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'tafser-al8asheya');
        this.storage.get('tafser-al8asheya-mp3').then(link => {
            if(link){
    ...
    streamaudio(url: string){
        this.storage.set('tafser-al8asheya-mp3', url);
        var options = {
            bgColor: "#000000",
            bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
            bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//     try {
//         request.open('GET', 'assets/aldurus-mp3/tafser/tafser-al8asheya.xml', false);
//         request.send(null);
//     } catch (err) {
//         return '';
//     }
