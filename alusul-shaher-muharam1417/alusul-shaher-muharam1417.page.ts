...
@Component({
  selector: 'app-alusul-shaher-muharam1417',
  templateUrl: './alusul-shaher-muharam1417.page.html',
  styleUrls: ['./alusul-shaher-muharam1417.page.scss'],
})
export class AlusulShaherMuharam1417Page implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-muharam-1417&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-shaher-muharam1417');
        this.storage.get('alusul-shaher-muharam1417-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-shaher-muharam1417-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-shaher-muharam1417-mp3').then((link) => {
            var options = {
