...
@Component({
  selector: 'app-alusul-jamade-awal',
  templateUrl: './alusul-jamade-awal.page.html',
  styleUrls: ['./alusul-jamade-awal.page.scss'],
})
export class AlusulJamadeAwalPage implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-jamade-awal-1416&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-jamade-awal');
        this.storage.get('alusul-jamade-awal-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-jamade-awal-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-jamade-awal-mp3').then((link) => {
            var options = {
