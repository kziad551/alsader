...
@Component({
  selector: 'app-tafser-alalak',
  templateUrl: './tafser-alalak.page.html',
  styleUrls: ['./tafser-alalak.page.scss'],
})
export class TafserAlalakPage implements OnInit {

private data:any = [];
  public title:any[] = [];
  public content:any[] = [];
  ngOnInit() {
      const url ='https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=mohadarat-tafsir-tafsir-alalak&populate=*'
  ...
  ionViewWillEnter(){
    ...
    this.storage.set('page-aldurus', 'tafser-alalak');
    this.storage.get('tafser-alalak-mp3').then(link => {
      if(link){
  ...
  streamaudio(url: string){
    this.storage.set('tafser-alalak-mp3', url);
    var options = {
      bgColor: "#000000",
      bgImage: encodeURI('https://alsader.net/wp-content/uploads/2021/04/الدروسnew-.jpg'),
      bgImageScale: "fit", // other valid values: "stretch", "aspectStretch"
...
//   try {
//     request.open('GET', 'assets/aldurus-mp3/tafser/tafser-alalak.xml', false);
//     request.send(null);
//   } catch (err) {
//     return '';
//   }
