...
@Component({
  selector: 'app-alusul-rabe3-altane1418',
  templateUrl: './alusul-rabe3-altane1418.page.html',
  styleUrls: ['./alusul-rabe3-altane1418.page.scss'],
})
export class AlusulRabe3Altane1418Page implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-rabi3-tene-1418&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-rabe3-altane1418');
        this.storage.get('alusul-rabe3-altane1418-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-rabe3-altane1418-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-rabe3-altane1418-mp3').then((link) => {
            var options = {
