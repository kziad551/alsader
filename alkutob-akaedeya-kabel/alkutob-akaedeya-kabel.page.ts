...
@Component({
  selector: 'app-alkutob-akaedeya-kabel',
  templateUrl: './alkutob-akaedeya-kabel.page.html',
  styleUrls: ['./alkutob-akaedeya-kabel.page.scss'],
})
export class AlkutobAkaedeyaKabelPage implements OnInit {
  ...
  this.storage.set('page', 'alkutob-akaedeya-kabel');
    this.storage.get('pdfLink9').then(link => {
      if(link){
  ...
  getlink(link:string){
    this.storage.get('pdfLink9').then(value => {
      this.storage.remove(value);
    });
    this.storage.set('pdfLink9', link);
  }
  forwardLink(){
    this.storage.get('pdfLink9').then((link) => {
  ...
  ngOnInit() {
    const url= 'https://strapi.alsader.net/api/pdfs?filters[pdf_category][title][$eq]=aakaed-kabl-estshhad&populate=*'
...
this.loadingController.dismiss();          })
  ...
  downloadKK(url: string, title: string) {
