...
@Component({
  selector: 'app-alusul-shawal1417',
  templateUrl: './alusul-shawal1417.page.html',
  styleUrls: ['./alusul-shawal1417.page.scss'],
})
export class AlusulShawal1417Page implements OnInit {
 ...
 ngOnInit() { 
  ...
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-shawal-1417&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-shawal1417');
        this.storage.get('alusul-shawal1417').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-shawal1417', link);
    }
    forwardLink(){
        this.storage.get('alusul-shawal1417').then((link) => {
            var options = {
