...
@Component({
  selector: 'app-alusul-zelka3da1419',
  templateUrl: './alusul-zelka3da1419.page.html',
  styleUrls: ['./alusul-zelka3da1419.page.scss'],
})
export class AlusulZelka3da1419Page implements OnInit {
...
constructor(private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-zelka3da1419');
        this.storage.get('alusul-zelka3da1419-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-zelka3da1419-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-zelka3da1419-mp3').then((link) => {
            var options = {
...
function getXMLDataVoice( itemname:string ) {
  ...
  try {
    request.open('GET', 'assets/alusul-1419-mp3/zelka3da-mp3.xml', false);
    request.send(null); 
  } catch (err) {  
    return '';
  }
