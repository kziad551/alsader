...
@Component({
  selector: 'app-mualafat-ketab-bay3',
  templateUrl: './mualafat-ketab-bay3.page.html',
  styleUrls: ['./mualafat-ketab-bay3.page.scss'],
})
export class MualafatKetabBay3Page implements OnInit {
  ...
  constructor(private http: HttpClient,private document: DocumentViewer,private transfer: FileTransfer,private fileOpener: FileOpener, private file: File,private storage: Storage,private platform: Platform,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page', 'mualafat-ketab-bay3.');
        this.storage.get('pdfLink15').then(link => {
            if(link){
  ...
  ngOnInit() {
    ...
    const url= 'https://strapi.alsader.net/api/pdfs?filters[pdf_category][title][$eq]=kitab-al-bay3&populate=*'
    ...
    getlink(link:string){
        this.storage.set('pdfLink15', link);
    }
    forwardLink(){
        this.storage.get('pdfLink15').then((link) => {
