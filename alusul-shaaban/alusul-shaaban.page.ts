...
@Component({
  selector: 'app-alusul-shaaban',
  templateUrl: './alusul-shaaban.page.html',
  styleUrls: ['./alusul-shaaban.page.scss'],
})
export class AlusulShaabanPage implements OnInit {
 ...
 ngOnInit() { 
  ...
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-shaaban-1416&populate=*'
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-shaaban');
        this.storage.get('alusul-shaaban-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-shaaban-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-shaaban-mp3').then((link) => {
            var options = {
