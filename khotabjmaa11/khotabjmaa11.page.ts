...
@Component({
  selector: 'app-khotabjmaa11',
  templateUrl: './khotabjmaa11.page.html',
  styleUrls: ['./khotabjmaa11.page.scss'],
})
export class Khotabjmaa11Page implements OnInit {
  private data:any = [];  
 public content:any[] = [];
 ...
 ngOnInit() { 
      this.storage.get('language-usingg').then(linkd => {
       if( linkd == 'ar'){
          this.data = [];
          this.content = [];
          const url= 'https://strapi.alsader.net/api/khotab-al-jomaa-docs?filters[id][$eq]=11?filters[khotab-al-jomaa-doc-cats][title][$eq]=khotab-docs&locale=ar-IQ'
          ...
          this.data = [];
          this.content = [];
          const url= 'https://strapi.alsader.net/api/khotab-al-jomaa-docs?filters[id][$eq]=56?filters[khotab-al-jomaa-doc-cats][title][$eq]=khotab-docs&locale=fa-IR'
          this.http.get(url).subscribe((res)=>{
            this.data = res
            var i =0;
            for ( i=0; i< this.data.data.length; i++ ) {
